...
export const useDashboardViewModel = (initialData?: { talents: any[], insights: any[] }) => {
  ...
  useEffect(() => {
    ...
    if (dashboard.talents.length === 0 && dashboard.insights.length === 0 && !dashboard.isLoading && !initialData) {
  ...
  }, [dispatch, dashboard.talents.length, dashboard.insights.length, dashboard.isLoading, identity.data, identity.isLoading, initialData]);

  // Derive identity alignment and focus
  const identityData = identity.data;
  const identityAlignment = identityData && identityData.goals.length > 0
    ? Math.round(identityData.goals.reduce((acc, goal) => acc + goal.progress, 0) / identityData.goals.length)
    : 0;

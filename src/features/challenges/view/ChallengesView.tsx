...
import { Target, Clock, Zap, Trophy, ArrowRight, CheckCircle } from "lucide-react"
...
export function ChallengesView({ initialData }: { initialData?: any }) {
  const { data, isLoading } = useChallengesViewModel();
  ...
  const { active: activeChallenges, available: availableChallenges, completed: completedChallenges } = activeData;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          {activeChallenges.map((challenge: any) => (
            ...
            >
              <div className="space-y-6">
                <ProgressBar 
                  value={((challenge.progress || 0) / (challenge.total || 1)) * 100} 
                  showValue 
                  label={`Progress: ${challenge.progress}/${challenge.total} days`}
                />

                <div className="rounded-md bg-secondary/30 p-4 border border-border">
                  <p className="mb-3 font-semibold text-foreground">Daily Tasks</p>
                  <div className="grid gap-3 md:grid-cols-2">
                    {challenge.tasks?.map((task: any, index: number) => (
                      <div key={index} className="flex items-center gap-2">
                        <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${task.completed ? "bg-primary" : "border border-border bg-background"}`}>
                          {task.completed && <CheckCircle className="h-3.5 w-3.5 text-primary-foreground" />}
                        </div>
                ...
                <Button className="w-full shadow-lg shadow-primary/20">Continue Today's Challenge</Button>
              </div>
            </DashboardCard>
          ))}
        </div>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-foreground">Available Challenges</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {challenge.benefits?.map((benefit: string) => (
                    <Badge key={benefit} variant="secondary" className="rounded-md text-[10px] uppercase font-bold tracking-wider">
                      {benefit}
                    </Badge>
                  ))}
                </div>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-foreground">Completed Challenges</h2>
        <div className="grid gap-4 md:grid-cols-2">
          {completedChallenges.map((challenge: any) => (
            <div key={challenge.title} className="flex items-center justify-between rounded-lg border border-border bg-secondary/20 p-4 transition-colors hover:bg-secondary/30">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Completed {challenge.completedDate}</p>
                </div>
              </div>
